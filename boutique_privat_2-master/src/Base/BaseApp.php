...
class BaseApp
{
    ...
    {
        return self::setNewClass('Base\Payment\Payment', [self::getListPayment()->{$id}, $id]);
    }
     ...
     * @return bool|mixed
     */
    public static function getPurchasable($id)
    {
        return self::setNewClass('Base\Purchasable\Purchasable', [self::getListPurchasable()->{$id}, $id]);
    }
     ...
     */
    public static function crateClient(array $options = [])
    {
        $options = self::applyDefaultClientOptions($options);
     ...
     * @return array
     */
    protected static function applyDefaultClientOptions(array $options)
    {
        $config = BaseApp::getConfigOptions();
        if ($config->sslVerify === null)
        {
            ...
            $verify = "src/Base/Http/" . $bundleFileName;
        }
        else
        {
        ...
        $options['headers']['User-Agent'] = 'Base (' . $config->baseUrl . ')';
